...
const PreviewPage = () => {
  const { invoiceData, setInvoiceData, selectedTemplate, setSelectedTemplate, baseURL } =
  ...
  const handleGeneratePaymentLink = async () => {
    if (!invoiceData.id) {
      toast.error("Please save the invoice first to generate a payment link.");
      return;
    }
    try {
      setGeneratingLink(true);
      const token = await getToken();
      const res = await fetch(`${baseURL}/payments/create-link/${invoiceData.id}`, {
        method: "POST",
        headers: {
      ...
      if (data.status === "success") {
        setInvoiceData(prev => ({ ...prev, paymentLink: data.paymentLink }));
        ...
        await saveInvoice(baseURL, payload, token);
      }
    } catch (error) {
      console.error(error);
    ...
    >
      <div className="d-flex flex-column align-items-center mb-4 gap-3">
        ...
        <div className="d-flex flex-wrap justify-content-center gap-2">
          ...
          {invoiceData.id && !invoiceData.paymentLink && (
            <button
              className="btn btn-warning d-flex align-items-center justify-content-center"
              onClick={handleGeneratePaymentLink}
              disabled={generatingLink}
            >
              {generatingLink && <Loader2 className="me-2 spin-animation" size={18} />}
              {generatingLink ? "Generating..." : "Generate Payment Link"}
            </button>
          )}
          {invoiceData.paymentLink && (
            <a href={invoiceData.paymentLink} target="_blank" rel="noreferrer" className="btn btn-success">
              Pay Now Link
            </a>
          )}
        </div>
