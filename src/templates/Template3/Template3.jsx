    ...
    <div className="template3 mx-auto my-4 p-5 w-800 bg-white shadow-sm border border-light">
      ...
      <div className="row g-4 mb-5" style={{ fontSize: "13px", lineHeight: "1.6" }}>
        <div className="col-sm-4">
          <span className="small text-muted text-uppercase d-block mb-2 tracking-wider" style={{ fontSize: "9px" }}>{t.billedFrom}</span>
        ...
        <div className="col-sm-4">
          <span className="small text-muted text-uppercase d-block mb-2 tracking-wider" style={{ fontSize: "9px" }}>{t.billedTo}</span>
        ...
        <div className="col-sm-4 text-sm-end text-start">
          ...
          <p className="mb-1 text-secondary"><strong>{t.invoiceNo}:</strong> <span className="text-dark fw-bold">{data.invoiceNumber}</span></p>
          <p className="mb-1 text-secondary"><strong>{t.date}:</strong> <span className="text-dark">{data.invoiceDate}</span></p>
          <p className="mb-0 text-secondary"><strong>{t.dueDate}:</strong> <span className="text-dark fw-bold" style={{ color: themeColor }}>{data.paymentDate}</span></p>
        </div>
      </div>
        ...
        <table className="table table-borderless alignment-middle border-top border-bottom py-2" style={{ fontSize: "13px" }}>
          <thead>
            <tr className="border-bottom text-uppercase" style={{ fontSize: "11px", letterSpacing: "0.06em" }}>
              <th className="p-3 fw-bold text-dark text-start">{t.description}</th>
              <th className="p-3 text-center fw-bold text-dark" style={{ width: "10%" }}>{t.qty}</th>
              <th className="p-3 text-end fw-bold text-dark" style={{ width: "20%" }}>{t.rate}</th>
              <th className="p-3 text-end fw-bold text-dark" style={{ width: "20%" }}>{t.amount}</th>
            </tr>
          </thead>
          <tbody>
        ...
        <div className="col-md-5">
          <div className="p-2" style={{ fontSize: "13px" }}>
            <div className="d-flex justify-content-between py-1">
              ...
              <span className="text-dark fw-medium">{data.currencySymbol}{subtotal.toFixed(2)}</span>
            </div>
            {data.tax > 0 && (
              <div className="d-flex justify-content-between py-1">
                <span className="text-secondary">{t.tax} ({data.tax}%)</span>
                <span className="text-dark fw-medium">{data.currencySymbol}{taxAmount.toFixed(2)}</span>
              </div>
            )}
            <hr className="my-2" />
            <div className="d-flex justify-content-between fw-bold align-items-center py-1" style={{ fontSize: "16px" }}>
              <span className="text-dark text-uppercase tracking-wider">{t.total}</span>
              <span className="text-dark">{data.currencySymbol}{total.toFixed(2)}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="row g-4 mt-2 border-top pt-4" style={{ fontSize: "12px", lineHeight: "1.6" }}>
        <div className="col-md-6">
          {(data.accountName || data.accountNumber || data.accountIfscCode) && (
            <div>
              <h6 className="fw-bold mb-2 text-dark text-uppercase tracking-wider" style={{ fontSize: "9px" }}>{t.bankDetails}</h6>
              {data.accountName && <p className="mb-1 text-muted"><strong>{t.holder || "Account Holder"}:</strong> {data.accountName}</p>}
              {data.accountNumber && <p className="mb-1 text-muted"><strong>{t.number || "Account Number"}:</strong> {data.accountNumber}</p>}
              {data.accountIfscCode && <p className="mb-0 text-muted"><strong>{t.ifsc || "IFSC Code"}:</strong> {data.accountIfscCode}</p>}
            </div>
          )}
        </div>
        <div className="col-md-6">
          {data.notes && (
            <div>
              <h6 className="fw-bold mb-2 text-dark text-uppercase tracking-wider" style={{ fontSize: "9px" }}>{t.terms}</h6>
